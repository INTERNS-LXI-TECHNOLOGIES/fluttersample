...
class ProfilePageState extends State<ProfilePage> {
  ...
  final GlobalKey<FormBuilderState> _fbKeyProfilePage = GlobalKey<FormBuilderState>();
  ...
  Widget build(BuildContext context) {
    ...
    var loggedInUser = _authBloc.loggedInUser;
    return Scaffold(
        ...
        child: BlocBuilder<AuthBloc, AuthState>(
          ...
          builder: (context, outcomeState) {
            ...
            if (outcomeState is ErrorAuthState) {
              ...
              return AlertDialog(
                title: Text('Error'),
                content: Text('Picture is too large ${outcomeState.error}'),
                actions: <Widget>[
                  FlatButton(
                    child: Text('Sorry'),
                    onPressed: () {
        ...
        FormBuilder(
          ...
          child: Column(
            children: <Widget>[
              ...
              FormBuilderTextField(
                attribute: 'login',
                initialValue: loggedInUser.login,
                readOnly: true,
                decoration: const InputDecoration(
              ...
              FormBuilderImagePicker(
                ...
                maxImages: 1,
                decoration: const InputDecoration(
                  labelText: 'Profile Picture [Max Size 192 x 102 Pixels]',
                ),
              ),
              SizedBox(height: 15),
            ],
              ...
              child: MaterialButton(
                ...
                onPressed: () {
                  ...
                  if (_fbKeyProfilePage.currentState.saveAndValidate()) {
                      ...
                      ..login = _fbKeyProfilePage.currentState.value['login'].toString()
                      ..email = _fbKeyProfilePage.currentState.value['email']
                      ..firstName = _fbKeyProfilePage.currentState.value['firstname']
                      ..lastName = _fbKeyProfilePage.currentState.value['lastname'] ;

                    // when image is not selected
                    try {
                      if ((_fbKeyProfilePage.currentState.value['imageUrl'] != null) &&
                          (_fbKeyProfilePage.currentState.value['imageUrl'][0] != null)) {
                        debugPrint(_fbKeyProfilePage.currentState.value['imageUrl'][0]
                            .toString());
                        String bytes = base64Encode(
                            (_fbKeyProfilePage.currentState.value['imageUrl'][0])
                                .readAsBytesSync()).toString();
